// ============================================================
// TOWER SIEGE — Mobile Controller (Socket.IO)
// Side-View Pose: Jump Detection + Body Alignment + Lean Deploy
...
const JUMP_THRESHOLD = 0.035;
const JUMP_COOLDOWN = 400;
let lastJumpTime = 0;
let emaHipY = 0;
const HIP_EMA_ALPHA = 0.3;
let isCalibrated = false;

// ── Body Alignment (Side View) ──────────────────────────────
let emaAlignAngle = 180;
const ALIGN_EMA_ALPHA = 0.25;
const ALIGN_GOOD_MIN = 160;
const ALIGN_GOOD_MAX = 195;
...
function checkSecureContext() {
    ...
    setStatusError('HTTPS Required',
        `Camera needs a secure connection.\nUse: https://${location.hostname}:${location.port}/tower-siege/controller.html`);
    return false;
}
...
async function initCamera() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        setStatusError('Camera Not Available',
            window.isSecureContext
                ...
                : `HTTPS required! Open:\nhttps://${location.hostname}:${location.port}/tower-siege/controller.html`);
        return false;
    }
    ...
    try {
        ...
        if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
            ...
            msg = 'HTTPS Required';
            sub = `Open: https://${location.hostname}:${location.port}/tower-siege/controller.html`;
        }
...
function initSocket() {
    ...
    socket.on('connect', () => {
        ...
        if (sessionId) {
            socket.emit('join-session', { sessionId }, (response) => {
                ...
                team = response.team || 'blue';
                teamBadge.textContent = `🏰 ${team.toUpperCase()} TEAM`;
                teamBadge.className = `badge ${team}`;
                console.log('[Socket] Joined as player', response.playerNum, 'team:', team);
            });
        } else {
    ...
    socket.on('game-started', (data) => {
        ...
        gameStatus.textContent = `⏱ ${data?.timer || 120}s — DO JUMPS!`;
...
function processHeadDirection(lm) {
    ...
    if (currentDirection !== 'center' && (now - lastNodTime) > NOD_COOLDOWN) {
        ...
        if (socket && socket.connected) {
            socket.emit('ts-manual-deploy', { direction: currentDirection });
        }
...
function processLandmarks(landmarks) {
    ...
    if (ankle && (ankle.visibility || 0) > 0.15) {
        ...
        const alignPct = Math.max(0, Math.min(100, 100 - (deviation * 3)));
    ...
    // ── JUMP DETECTION ────────────────────────────────────────
    const hipRise = restHipY - emaHipY;
    const now = Date.now();
    ...
    if (jumpState === 'grounded' && hipRise > JUMP_THRESHOLD) {
        ...
        if ((now - lastJumpTime) > JUMP_COOLDOWN) {
            ...
            if (socket && socket.connected && isGameActive) {
                socket.emit('ts-pushup', { count: pushups });
            }
    ...
    // Re-calibrate slowly (drift correction)
...
readyBtn.addEventListener('click', () => {
    if (socket && socket.connected) {
        socket.emit('ts-player-ready', { deployMode });
...
async function init() {
    ...
    // Step 2: Connect Socket.IO (non-blocking)
